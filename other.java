...
//    }
    ...
    void dfs(int i, int level,int[][] triangle,int val){
        if(level==triangle.length-1){
            res = Math.max(res,val+triangle[level][2*i+1]);
            res = Math.max(res,val+triangle[level][2*i+2]);
        } else {
            dfs(i*2+1,level+1,triangle,triangle[level][i]+val);
            dfs(i*2+2,level+1,triangle,triangle[level][i]+val);
        }
    }
